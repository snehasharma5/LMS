...
from web_app.models import RegisterModel, Student, Expert, Institute, Industry
...
class IndustryRegisterForm(UserCreationForm):
    ...
    def save(self):
        ...
        if self.cleaned_data.get('last_name') == "#":
            ...
            user.last_name = self.cleaned_data.get('last_name')
        user.save()
        industry = Industry.objects.create(user=user)
        industry.address = self.cleaned_data.get('address')
        return user

    def __init__(self, *args, **kwargs):
        super(IndustryRegisterForm, self).__init__(*args, **kwargs)
